...
- **`main`**: Production branch. Pushes here trigger AWS deploy. No direct pushes allowed.
- **`dev`**: Default branch. All feature branches merge here first. Branch protection enabled with same status checks as `main`.

```bash
# Feature development (always branch from dev)
git checkout dev && git pull
git checkout -b feature/your-change
# make changes, commit
git push -u origin feature/your-change
gh pr create --title "Your change"    # PR targets dev (default)
# checks pass → merge into dev

# Production release (dev → main)
gh pr create --base main --head dev --title "Release: description"
# checks pass → merge into main → auto-deploys to AWS
```

Required status checks before merge (both `dev` and `main`): `Backend Build & Test`, `Frontend Build & Lint`.
