...
public interface InvoiceBillRepository extends ScidRepository<InvoiceBill> {
    ...
    /** Wipe all synthetic (auto-generated) cash bills for a shift. Called before regeneration at shift close. */
    @Modifying
    @Transactional
    @Query("DELETE FROM InvoiceBill ib WHERE ib.shiftId = :shiftId AND ib.autoGenerated = true")
    int deleteByShiftIdAndAutoGeneratedTrue(@Param("shiftId") Long shiftId);
