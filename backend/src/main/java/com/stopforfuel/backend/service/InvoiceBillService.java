...
public class InvoiceBillService {
    ...
    public InvoiceBill createInvoice(InvoiceBill invoice) {
        ...
        if (invoice.getShiftId() == null) {
            ...
            validateTargetShift(invoice.getShiftId(), EnumSet.of(ShiftStatus.OPEN, ShiftStatus.REVIEW));
        }
    ...
    /**
     * Validates that the caller may post or move an invoice to the given shift:
     * shift exists, scid matches the caller, and current status is in the allowed set.
     * Returns the loaded shift for further use by the caller.
     */
    private Shift validateTargetShift(Long shiftId, EnumSet<ShiftStatus> allowedStatuses) {
        Shift target = shiftService.findById(shiftId)
                .orElseThrow(() -> new BusinessException("Target shift " + shiftId + " not found"));
        if (!java.util.Objects.equals(target.getScid(), SecurityUtils.getScid())) {
            throw new BusinessException("Target shift does not belong to your company");
        }
        if (!allowedStatuses.contains(target.getStatus())) {
            throw new BusinessException(
                    "Cannot post to shift #" + shiftId + " — it is " + target.getStatus() +
    ...
    /**
     * Admin-only: move an existing invoice to a different shift and adjust its bill_date.
     * Used to fix mis-shifted bills (e.g. cashier created a back-dated invoice on the wrong shift).
     *
     * Constraints:
     *   - Target shift must be OPEN/REVIEW, or CLOSED with its closing report still DRAFT (so a
     *     subsequent Recompute on the target report picks up the moved bill cleanly).
     *   - Source shift's report must NOT be FINALIZED (un-finalize it first if it is); otherwise
     *     moving the bill out would leave the source report's cached totals stale.
     *   - Auto-generated synthetic cash bills are not movable — they're wiped/regenerated by the
     *     residual-cash flow at shift close.
     *   - newBillDate must fall inside the target shift's [startTime, endTime] window.
     */
    /**
     * Admin-only: set an invoice's shift_id without changing its bill_date. Used by the orphan-bill
     * recovery UI to assign a covering shift to a NULL-shift bill (the 2026-04-03 bulk-import case).
     * Unlike {@link #moveInvoice}, this preserves bill_date — the bill_date is canonical and the
     * shift just records which session it belongs to. Validates target shift exists in the same
     * tenant; allows any non-deleted shift status (OPEN/REVIEW/CLOSED/RECONCILED) since we may
     * need to assign to an old shift for back-dated recovery.
     */
    @Transactional
    public InvoiceBill setShift(Long invoiceId, Long targetShiftId) {
        InvoiceBill invoice = repository.findByIdAndScid(invoiceId, SecurityUtils.getScid())
                .orElseThrow(() -> new ResourceNotFoundException("Invoice not found: " + invoiceId));
        if (invoice.isAutoGenerated()) {
            throw new BusinessException(
                    "Cannot set shift on auto-generated cash bill #" + invoice.getBillNo());
        }
        if (targetShiftId == null) {
            throw new BusinessException("Target shift is required");
        }
        Shift target = shiftService.findById(targetShiftId)
                .orElseThrow(() -> new ResourceNotFoundException("Shift not found: " + targetShiftId));
        if (!java.util.Objects.equals(target.getScid(), SecurityUtils.getScid())) {
            throw new BusinessException("Shift does not belong to your tenant");
        }
    ...
    @Transactional
    public InvoiceBill moveInvoice(Long invoiceId, Long targetShiftId, LocalDateTime newBillDate) {
        InvoiceBill invoice = repository.findByIdAndScid(invoiceId, SecurityUtils.getScid())
                .orElseThrow(() -> new ResourceNotFoundException("Invoice not found: " + invoiceId));

        if (invoice.isAutoGenerated()) {
            throw new BusinessException(
                    "Cannot move auto-generated cash bill #" + invoice.getBillNo() +
        ...
        if (newBillDate == null) {
            throw new BusinessException("New bill date is required");
        }

        Long sourceShiftId = invoice.getShiftId();
        if (java.util.Objects.equals(sourceShiftId, targetShiftId)) {
            throw new BusinessException("Invoice is already on shift #" + targetShiftId);
        }

        // Source check: refuse if source report is FINALIZED (would leave stale cached totals).
        if (sourceShiftId != null) {
            shiftClosingReportRepository.findByShift_Id(sourceShiftId).ifPresent(srcReport -> {
                if ("FINALIZED".equals(srcReport.getStatus())) {
                    throw new BusinessException(
                            "Cannot move invoice out of shift #" + sourceShiftId +
                            " — its closing report is FINALIZED. Un-finalize the source report first.");
                }
            });
        }

        // Target check: status must be OPEN, REVIEW, or CLOSED-with-DRAFT-report.
        Shift target = validateTargetShift(
                targetShiftId,
                EnumSet.of(ShiftStatus.OPEN, ShiftStatus.REVIEW, ShiftStatus.CLOSED));
        if (target.getStatus() == ShiftStatus.CLOSED) {
            shiftClosingReportRepository.findByShift_Id(targetShiftId).ifPresent(tgtReport -> {
                if ("FINALIZED".equals(tgtReport.getStatus())) {
                    throw new BusinessException(
                            "Cannot move invoice into shift #" + targetShiftId +
                            " — its closing report is FINALIZED. Un-finalize the target report first.");
                }
            });
        }

        // newBillDate must be inside target window.
        LocalDateTime windowStart = target.getStartTime();
        LocalDateTime windowEnd = target.getEndTime() != null ? target.getEndTime() : LocalDateTime.now();
        if (windowStart != null && newBillDate.isBefore(windowStart)) {
            throw new BusinessException(
                    "Bill date " + newBillDate + " is before shift #" + targetShiftId +
                    " start (" + windowStart + ").");
        }
        if (newBillDate.isAfter(windowEnd)) {
            throw new BusinessException(
                    "Bill date " + newBillDate + " is after shift #" + targetShiftId +
                    " end (" + windowEnd + ").");
        }
