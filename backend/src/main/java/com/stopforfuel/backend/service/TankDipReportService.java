...
public class TankDipReportService {
    ...
    public byte[] generatePdf(List<TankInventory> data, LocalDate fromDate, LocalDate toDate, String tankName) {
        try {
            ...
            Paragraph title = new Paragraph("Tank Dip Readings Report", reportTitleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            doc.add(title);

            String displayTank = tankName.replace("_", " ");
            Paragraph meta = new Paragraph("Tank: " + displayTank + "  |  Period: " + fromDate.format(DATE_FMT) + " to " + toDate.format(DATE_FMT), subFont);
            ...
            // Table
            float[] widths = {4f, 8f, 12f, 10f, 7f, 9f, 9f, 7f, 9f, 12f};
            PdfPTable table = new PdfPTable(widths);
            table.setWidthPercentage(100);

            String[] headers = {"#", "DATE", "TANK", "PRODUCT", "OPEN DIP", "OPEN STOCK (L)", "INCOME (L)", "CLOSE DIP", "CLOSE STOCK (L)", "DAILY SALES (L)"};
            ...
            for (TankInventory inv : data) {
                ...
                addCell(table, inv.getTank().getName(), boldCellFont, Element.ALIGN_CENTER, rowBg, borderColor);
                addCell(table, inv.getTank().getProduct() != null ? inv.getTank().getProduct().getName() : "-", cellFont, Element.ALIGN_CENTER, rowBg, borderColor);
                addCell(table, inv.getOpenDip() != null ? inv.getOpenDip() : "-", cellFont, Element.ALIGN_CENTER, rowBg, borderColor);
                ...
                addCell(table, inv.getCloseDip() != null ? inv.getCloseDip() : "-", cellFont, Element.ALIGN_CENTER, rowBg, borderColor);
                addCell(table, formatNum(inv.getCloseStock()), cellFont, Element.ALIGN_RIGHT, rowBg, borderColor);

                double sale = inv.getSaleStock() != null ? inv.getSaleStock() : 0;
    ...
    public byte[] generateExcel(List<TankInventory> data, LocalDate fromDate, LocalDate toDate, String tankName) {
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            reportCell.setCellValue("Tank Dip Readings Report");
            ...
            String displayTank = tankName.replace("_", " ");
            metaCell.setCellValue("Tank: " + displayTank + "  |  Period: " + fromDate.format(DATE_FMT) + " to " + toDate.format(DATE_FMT));
            ...
            for (TankInventory inv : data) {
                ...
                c2.setCellValue(inv.getTank().getName());
                c2.setCellStyle(cellStyle);

                XSSFCell c3 = row.createCell(3);
                c3.setCellValue(inv.getTank().getProduct() != null ? inv.getTank().getProduct().getName() : "-");
                c3.setCellStyle(cellStyle);

                XSSFCell c4 = row.createCell(4);
                c4.setCellValue(inv.getOpenDip() != null ? inv.getOpenDip() : "-");
                ...
                c7.setCellValue(inv.getCloseDip() != null ? inv.getCloseDip() : "-");
                c7.setCellStyle(cellStyle);

                XSSFCell c8 = row.createCell(8);
                c8.setCellValue(inv.getCloseStock() != null ? inv.getCloseStock() : 0);
                c8.setCellStyle(numStyle);

                double sale = inv.getSaleStock() != null ? inv.getSaleStock() : 0;
                XSSFCell c9 = row.createCell(9);
                c9.setCellValue(sale);
                c9.setCellStyle(salesStyle);
                totalSales += sale;
            }
            ...
            XSSFCell sumSales = sumRow.createCell(9);
