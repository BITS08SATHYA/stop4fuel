...
# ============================================================
build_frontend() {
    step "Building frontend Docker image (API_URL=${NEXT_PUBLIC_API_URL})..."
    docker build \
...
deploy_ec2() {
    ...
    ${SSH_CMD} << 'REMOTE_EOF'
set -e
ECR_BASE="971422715802.dkr.ecr.us-east-1.amazonaws.com"
COMPOSE_FILE="/home/ubuntu/docker-compose.prod.yml"
...
docker compose -f ${COMPOSE_FILE} up -d --force-recreate --pull always
