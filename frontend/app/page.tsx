                        ...
                        {stats.tankStatuses.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {stats.tankStatuses.map((tank) => {
                                    ...
                                    if (pct < 20) {
                                        barColor = "bg-red-500"; statusColor = "text-red-500"; statusLabel = "Critical";
                                    } else if (pct < 50) {
                                        barColor = "bg-amber-500"; statusColor = "text-amber-500"; statusLabel = "Low";
                                    }
                                    return (
                                        <div key={tank.tankId} className="p-3 rounded-xl border border-border/50 bg-card/50">
                                            <div className="flex items-center justify-between mb-2">
                                                <div>
                                                    <p className="text-sm font-semibold text-foreground">{tank.tankName}</p>
                                                    <p className="text-[10px] text-muted-foreground">{tank.productName || "—"}</p>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <span className={`text-[10px] font-medium ${statusColor}`}>{statusLabel}</span>
                                                    {!tank.active && (
                                                        <span className="text-[10px] bg-red-500/10 text-red-500 px-1.5 py-0.5 rounded">Off</span>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex justify-between mt-1">
                                                <span className="text-[10px] text-muted-foreground">
                                                    {tank.currentStock.toLocaleString("en-IN", { maximumFractionDigits: 0 })} / {tank.capacity.toLocaleString("en-IN", { maximumFractionDigits: 0 })} L
                                                </span>
                                            </div>
                                        </div>
                                    );
